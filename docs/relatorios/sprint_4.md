...
| **Yan Guimarães** | Expansão da cobertura de testes para o campo de tags na API de conversas. Implementação de suporte a múltiplos formatos de entrada (listas e strings) no Serializer e correção de erros de validação (400 Bad Request). | [MR #37](https://gitlab.com/gces-ej/ej-application/-/merge_requests/37) | Aumentou a robustez da API garantindo polimorfismo no recebimento de dados. A correção previne falhas de integração e assegura que diferentes formatos de payload sejam processados corretamente. |
---
